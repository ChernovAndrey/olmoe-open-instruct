...
def get_next_word_predictions(model, tokenizer, prompts, candidate_token_ids=None, batch_size=1,
    ...
    def hook(module, input, output, layer_idx):
        ...
        gating_alphas[layer_idx].append(probabilities)  # Append per generation step

    hooks = []
    for layer_idx, layer in enumerate(model.model.layers):
        if hasattr(layer, 'mlp') and hasattr(layer.mlp, 'gate'):
            hook_fn = lambda module, input, output, idx=layer_idx: hook(module, input, output, idx)
    ...
    return predictions, probs, gating_alphas
